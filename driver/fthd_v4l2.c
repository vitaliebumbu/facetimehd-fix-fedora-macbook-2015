 ...
 *
 * Modified 2026 by vitaliebumbu <https://github.com/vitaliebumbu>:
 *   - Non-blocking buf_queue using a workqueue for async hardware
 *     submission (fixes deadlock that froze stream after one frame
 *     on kernel 6.15+).
 *   - Set buffer timestamps via ktime_get_ns() and sequence numbers
 *     so modern V4L2 consumers don't see a stalled stream.
 *   - Fixed start_streaming error path.
 *
...
/*
 * Workqueue function: submits pending buffers to the ISP hardware.
 * This runs WITHOUT the vb2 queue lock held, so the blocking
 * fthd_channel_wait_ready() call won't deadlock with DQBUF.
 */
static void fthd_buf_submit_work(struct work_struct *work)
{
	struct fthd_private *dev_priv = container_of(work, struct fthd_private, buf_work);
	...
	for (i = 0; i < FTHD_BUFFERS; i++) {
		...
		if (!dev_priv->vb2_queue.streaming)
			continue;
...
int fthd_send_h2t_buffer(struct fthd_private *dev_priv, struct h2t_buf_ctx *ctx)
{
	u32 entry;
	int ret;
...
/*
 * buf_queue: called by vb2 with q->lock held.
 * MUST NOT block — just mark the buffer as driver-queued and
 * schedule the workqueue to do the actual hardware submission.
 */
static void fthd_buffer_queue(struct vb2_buffer *vb)
{
	...
	if (vb->vb2_queue->streaming)
		schedule_work(&dev_priv->buf_work);
}
...
void fthd_buffer_return_handler(struct fthd_private *dev_priv, u32 offset, int size)
{
	...
	for(i = 0; i < list.count; i++) {
		...
		if (ctx->state == BUF_HW_QUEUED || ctx->state == BUF_DRV_QUEUED) {
			...
			vbuf = to_vb2_v4l2_buffer(ctx->vb);
...
static void fthd_stop_streaming(struct vb2_queue *vq)
{
	...
	/* Flush any pending async buffer submissions */
	flush_work(&dev_priv->buf_work);
...
int fthd_v4l2_register(struct fthd_private *dev_priv)
{
	...
	/* Initialize the buffer submission workqueue */
	INIT_WORK(&dev_priv->buf_work, fthd_buf_submit_work);
