 ...
 *   - Fixed variable shadowing in fthd_irq_work where the inner for
 *     loop reused the outer while loop's counter, breaking the
 *     safety bound.
 *   - Added buf_work workqueue cleanup on remove.
 *
...
static void fthd_irq_work(struct work_struct *work)
{
	...
	for (i = 0; i < 500; i++) {
		...
		for(j = 0; j < dev_priv->num_channels; j++) {
			chan = dev_priv->channels[j];
...
static void fthd_pci_remove(struct pci_dev *pdev)
{
	...
	cancel_work_sync(&dev_priv->buf_work);
